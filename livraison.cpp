...
QSqlQueryModel* Livraison::search(QString keyword) {
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;

    query.prepare("SELECT * FROM LIVRAISON WHERE NUM_SUIV LIKE :keyword OR ADDRESS LIKE :keyword");
...
QSqlQueryModel* Livraison::trierParDateSortie()
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QSqlQuery query;

    // Requête SQL pour trier par DATE_SORTIE en ordre décroissant
    query.prepare("SELECT * FROM LIVRAISON ORDER BY DATE_SORTIE DESC");

    if (!query.exec()) {
        qDebug() << "Erreur lors du tri par date de sortie:" << query.lastError().text();
        return nullptr;
    }
...
QMap<QString, int> Livraison::statistiquesLivraisonsParMois() const {
    QMap<QString, int> stats;

    // Vérifier que la connexion est bien ouverte
    QSqlDatabase db = QSqlDatabase::database();
    if (!db.isOpen() && !db.open()) {
        qDebug() << "La connexion à la base de données n'a pas pu être ouverte.";
        return stats;  // Retourner une map vide
    }

    // Requête SQL pour obtenir les dates de sortie des livraisons
    QSqlQuery query("SELECT DATE_SORTIE FROM livraison");
    if (query.exec()) {
        while (query.next()) {
            QDate dateSortie = query.value("DATE_SORTIE").toDate();
            // Vérifier si la date est valide
            if (dateSortie.isValid()) {
                QString moisAnnee = dateSortie.toString("MMMM yyyy");
                stats[moisAnnee]++;  // Incrémenter le nombre de livraisons pour le mois/année donné
            }
        }
    } else {
